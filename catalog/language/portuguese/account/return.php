<?php
// Heading 
$_['heading_title']      = 'Devoluções';

// Text
$_['text_account']       = 'Minha Conta';
$_['text_return']        = 'Informações da Devolução';
$_['text_return_detail'] = 'Detalhes da Devolução';
$_['text_description']   = '<p>Por favor, complete o formulário para obter um número de RMA (Retorno de Mercadoria Adquirida).</p>';
$_['text_order']         = 'Informações do Pedido';
$_['text_product']       = 'Informações do Produto e Motivo da Devolução';
$_['text_message']       = '<p>Obrigado. A sua solicitação de devolução foi enviada para o departamento responsável.</p><p> Receberá notificações via e-mail sobre a situação da sua solicitação.</p>';
$_['text_return_id']     = 'Devolução Nº:';
$_['text_order_id']      = 'Pedido Nº:';
$_['text_date_ordered']  = 'Data do Pedido:';
$_['text_status']        = 'Situação:';
$_['text_date_added']    = 'Adicionada em:';
$_['text_customer']      = 'Cliente:';
$_['text_comment']       = 'Comentários da Devolução';
$_['text_history']       = 'Histórico da Devolução';
$_['text_empty']         = 'Ainda não fez nenhuma solicitação de devolução!';
$_['text_error']         = 'A solicitação de devolução que procura não foi encontrada!';

// Column
$_['column_product']     = 'Produto';
$_['column_model']       = 'Modelo';
$_['column_quantity']    = 'Quantidade';
$_['column_price']       = 'Preço';
$_['column_opened']      = 'Em Aberto';
$_['column_comment']     = 'Comentário';
$_['column_reason']      = 'Motivo';
$_['column_action']      = 'Acção';
$_['column_date_added']  = 'Adicionado em';
$_['column_status']      = 'Situação';

// Entry
$_['entry_order_id']     = 'Pedido Nº:';
$_['entry_date_ordered'] = 'Data do Pedido:';
$_['entry_firstname']    = 'Nome:';
$_['entry_lastname']     = 'Apelido:';
$_['entry_email']        = 'E-mail:';
$_['entry_telephone']    = 'Telefone:';
$_['entry_product']      = 'Nome do Produto:';
$_['entry_model']        = 'Modelo do Produto:';
$_['entry_quantity']     = 'Quantidade:';
$_['entry_reason']       = 'Motivo da Devolução:';
$_['entry_opened']       = 'Embalagem Aberta:';
$_['entry_fault_detail'] = 'Defeitos e Outros Detalhes:';
$_['entry_captcha']      = 'Insira o código da imagem no campo abaixo:';

// Error
$_['error_order_id']     = 'O Nº do pedido é obrigatório!';
$_['error_firstname']    = 'O Nome deve ter entre 1 e 32 caracteres!';
$_['error_lastname']     = 'O Apelido deve ter entre 10 e 32 caracteres!!';
$_['error_email']        = 'O E-mail não é válido!';
$_['error_telephone']    = 'O Telefone deve ter entre 3 e 32 caracteres!';
$_['error_product']      = 'O Nome do produto deve ser maior que 3 e menor que 255 caracteres!!';
$_['error_model']        = 'O Modelo do produto deve ser maior que 3 e menor que 64 caracteres!';
$_['error_reason']       = 'Deve seleccionar um motivo para a devolução!';
$_['error_captcha']      = 'O código digitado não corresponda imagem!';
?>